  ...
  <div class="app-container">
    ...
    <el-table :data="list" border style="width: 100%">
      ...
      <el-table-column fixed="right" label="操作">
        <template slot-scope="scope">
          <el-button type="text"  size="small" @click="updateAuthStatus(scope.row.id,-1,scope.row.certificatesNo,scope.row.name)">驳回</el-button>
          <el-button type="text"  size="small" @click="updateAuthStatus(scope.row.id,2,scope.row.certificatesNo,scope.row.name)">通过</el-button>
...
export default {
  methods: {
    //用户审核用户
    updateAuthStatus(id,authStatus,idCard,name){

      if(authStatus==2){
        //点击的通过按钮，判断证件信息是否一致
        //发起第一个请求
        userinfoApi.check(idCard,name).then(resp=>{
          if(resp.message=='不一致'){
            //弹窗
            this.$confirm('该用户的姓名和证件不一致, 是否继续?', '提示', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              //发起第二个请求，依然让他通过
              userinfoApi.updateAuthStatus(id,authStatus).then(resp=>{
    ...
    clear(){
        this.searchObj = {authStatus:1} //后端永远查询authStatus=1的用户列表
        this.value1 = []
        this.fetchData()
    }
  },
  data() {
    return {
      //查询表单绑定的属性
      searchObj: {authStatus:1},//后端永远查询authStatus=1的用户列表
      list: [], //用户列表
      total: 0, //总记录数
      page: 1,
      limit: 5,
