...
export default {
  ...
  created() {
    //获取路由id
    const id = this.$route.params.id;//当前医院的id
    //调用方法，根据id查询医院详情
  ...
  methods: {
    ...
    fetchHospDetail(id) {
      hospApi.getHospById(id).then((response) => {
        this.hospital = response.data.item.hospital;//第一个table
        this.bookingRule = response.data.item.bookingRule;//第二个table
      });
    },
    //返回医院列表
    back() {
      this.$router.push({ path: "/hosp/hosplist" });
    },
  },
};
</script>